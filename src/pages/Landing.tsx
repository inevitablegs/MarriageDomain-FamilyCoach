...
export function Landing({ onNavigate }: LandingProps) {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-slate-50 font-sans text-slate-800">
      ...
      <div className="absolute top-0 -left-64 h-[500px] w-[500px] rounded-full bg-indigo-200/40 blur-[100px] opacity-70" />
      <div className="absolute top-40 -right-64 h-[600px] w-[600px] rounded-full bg-blue-200/40 blur-[120px] opacity-60" />
      <div className="absolute -bottom-64 left-1/2 h-[700px] w-[700px] -translate-x-1/2 rounded-full bg-rose-100/40 blur-[150px] opacity-50" />

      <div className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">

        {/* HERO SECTION */}
        <section className="animate-rise-in relative flex flex-col items-center justify-center text-center">
          <div className="inline-flex items-center gap-2 rounded-full border border-indigo-200/50 bg-indigo-50/50 px-4 py-1.5 text-xs font-bold uppercase tracking-wider text-indigo-700 shadow-sm backdrop-blur-sm">
            <Sparkles size={14} className="text-indigo-500" />
            Relationship Clarity System
          </div>

          <h1 className="mt-8 max-w-5xl text-5xl font-extrabold leading-[1.1] tracking-tight text-slate-900 sm:text-6xl lg:text-7xl">
          ...
          <div className="mt-10 flex flex-col gap-4 sm:flex-row sm:items-center justify-center w-full max-w-md sm:max-w-none">
            ...
            <button
              ...
              className="flex w-full items-center justify-center rounded-full border border-slate-200 bg-white/70 px-8 py-4 text-base font-bold text-slate-800 shadow-sm transition-all hover:bg-white hover:shadow-md sm:w-auto"
            >
              Fix My Relationship
            </button>
          </div>

          <p className="mt-6 text-sm font-medium text-slate-500 flex items-center gap-2 justify-center">
            <HeartHandshake size={14} className="text-slate-400" /> Trusted by thousands of couples & individuals
          </p>
        </section>

        {/* METRICS / STATS BENTO */}
        <section className="animate-rise-in mx-auto mt-24 max-w-5xl rounded-[2.5rem] premium-card p-2">
          <div className="grid overflow-hidden rounded-[2rem] sm:grid-cols-2 lg:grid-cols-3 bg-slate-50/50">
            <div className="p-10 border-b sm:border-b-0 sm:border-r border-slate-200/60 text-center">
              <div className="text-4xl font-extrabold text-indigo-600">85%</div>
              <div className="mt-2 text-sm font-bold text-slate-600 uppercase tracking-wide">Clarity within 1 Week</div>
            </div>
            <div className="p-10 border-b lg:border-b-0 lg:border-r border-slate-200/60 text-center">
              <div className="text-4xl font-extrabold text-blue-600">3x</div>
              <div className="mt-2 text-sm font-bold text-slate-600 uppercase tracking-wide">Fewer Major Conflicts</div>
            </div>
            <div className="p-10 text-center sm:col-span-2 lg:col-span-1">
              <div className="text-4xl font-extrabold text-sky-500">100%</div>
              <div className="mt-2 text-sm font-bold text-slate-600 uppercase tracking-wide">Private & Secure</div>
            </div>
          </div>
        </section>
        ...
        <section className="animate-rise-in mt-32 grid gap-10 lg:grid-cols-2 lg:items-center">
              ...
              {problemPoints.map((item) => (
                <div key={item} className="flex items-center gap-3">
                  <div className="flex-shrink-0 h-8 w-8 rounded-full bg-rose-100 flex items-center justify-center">
                    <CircleDot size={14} className="text-rose-600" />
                  </div>
          ...
          <div className="glass-effect rounded-3xl p-8 sm:p-12 relative overflow-hidden">
            <div className="absolute top-0 right-0 p-32 bg-amber-100/50 rounded-full blur-3xl -z-10 translate-x-1/2 -translate-y-1/2"></div>
            <h3 className="text-2xl font-bold text-slate-900 mb-6">We Give You a System - Not Just Advice.</h3>
            <ul className="space-y-4">
              {solutionPoints.map((item) => (
                <li key={item} className="flex items-start gap-4 bg-white/60 p-4 rounded-xl border border-white">
        ...
        <section className="mt-32">
          <div className="text-center max-w-2xl mx-auto">
            <h2 className="text-3xl font-extrabold text-slate-900">Simple. Structured. Effective.</h2>
            <p className="mt-4 text-slate-600 text-lg">Your path to relationship clarity takes just three steps.</p>
          </div>
        ...
        <section className="mt-32 grid gap-8 lg:grid-cols-2">
          <article className="premium-card p-10 relative overflow-hidden group border-2 border-transparent hover:border-indigo-100 transition-colors">
            ...
            <p className="text-sm font-bold uppercase tracking-widest text-indigo-600">Before Marriage</p>
            <h3 className="mt-3 text-3xl font-extrabold text-slate-900">Confused about your partner?</h3>
            <p className="mt-4 text-slate-600 leading-relaxed text-lg">Stop guessing. Get deep insights before making the biggest commitment of your life.</p>
            <ul className="mt-8 space-y-4 text-slate-700 font-medium">
              <li className="flex items-center gap-3"><span className="p-1 bg-indigo-50 rounded-full"><CheckCircle2 size={18} className="text-indigo-600" /></span> Compatibility Deep Scan</li>
              <li className="flex items-center gap-3"><span className="p-1 bg-indigo-50 rounded-full"><CheckCircle2 size={18} className="text-indigo-600" /></span> Red Flag Intelligence</li>
              <li className="flex items-center gap-3"><span className="p-1 bg-indigo-50 rounded-full"><CheckCircle2 size={18} className="text-indigo-600" /></span> Make Confident Decisions</li>
            </ul>
            <button
              onClick={() => onNavigate('auth-before')}
          ...
          <article className="premium-card p-10 relative overflow-hidden group border-2 border-transparent hover:border-emerald-100 transition-colors">
            ...
            <p className="text-sm font-bold uppercase tracking-widest text-emerald-600">After Marriage</p>
            <h3 className="mt-3 text-3xl font-extrabold text-slate-900">Facing issues in your relationship?</h3>
            <p className="mt-4 text-slate-600 leading-relaxed text-lg">Repair and rebuild together with structured assessment sessions and health tracking.</p>
            <ul className="mt-8 space-y-4 text-slate-700 font-medium">
              <li className="flex items-center gap-3"><span className="p-1 bg-emerald-50 rounded-full"><CheckCircle2 size={18} className="text-emerald-600" /></span> Couple Assessment Sessions</li>
              <li className="flex items-center gap-3"><span className="p-1 bg-emerald-50 rounded-full"><CheckCircle2 size={18} className="text-emerald-600" /></span> Conflict Risk Monitor</li>
              <li className="flex items-center gap-3"><span className="p-1 bg-emerald-50 rounded-full"><CheckCircle2 size={18} className="text-emerald-600" /></span> Rebuild Connection</li>
            </ul>
            <button
              onClick={() => onNavigate('auth-after')}
        ...
        <section className="mt-32 rounded-[3rem] bg-gradient-to-br from-slate-900 to-slate-800 p-12 text-center text-white shadow-2xl shadow-indigo-900/20 relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
              <button
                onClick={() => onNavigate('auth-before')}
                className="w-full sm:w-auto rounded-full bg-white px-8 py-4 font-bold text-slate-900 transition-transform hover:scale-105 shadow-xl"
              >
                Start Compatibility Check
              </button>
            </div>
        ...
        <footer className="mt-20 border-t border-slate-200/60 pt-8 pb-4 text-center">
          <div className="flex flex-wrap items-center justify-center gap-4 text-xs font-bold uppercase tracking-wider text-slate-400 mb-6">
          ...
          <p className="text-sm text-slate-500">© {new Date().getFullYear()} MarriageWise. Built for better relationship decisions.</p>
        </footer>
